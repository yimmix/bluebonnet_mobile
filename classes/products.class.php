...
*******************************************************************************/
	...
	class Products extends SqlQuery{
		...
		public function getFilterOptions($page_cnt,$cond,$kwd){
			...
			LEFT OUTER JOIN suplement_facts sf ON (sf.product_id = P.product_id)  WHERE  P.status='1' ";
				...
				$qry = " SELECT  DISTINCT  P.* from products P
				LEFT JOIN product_packages PP ON (PP.product_id = P.product_id)
				LEFT JOIN product_labels PL ON (PL.product_id = P.product_id)
				LEFT JOIN labels L ON (L.label_id = PL.label_id)
				LEFT JOIN suplement_facts SF ON (SF.product_id = P.product_id)
				LEFT JOIN suplement_fact_items SFI ON (SFI.suplement_fact_id = P.product_id)  WHERE P.status='1' AND  CONCAT_WS(
                ' ',P.sku, P.product_name,P.product_desc,SF.serving_size,SF.other_ingredients, SF.contains,SF.caution,
  L.label_name,SFI.ingredient_text, SFI.ingredient_text_2, SFI.section_name,PP.package_medium
  ) LIKE  '%".$kwd."%' AND CONCAT_WS(' ',P.sku, P.product_name,P.product_desc,SF.serving_size,SF.other_ingredients, SF.contains,SF.caution,
  L.label_name,SFI.ingredient_text, SFI.ingredient_text_2, SFI.section_name,PP.package_medium) RLIKE '[[:<:]]".$kwd."[[:>:]]'";
		...
		public function  cntSearchedproduct($cond,$kwd) {
			
			if($kwd != '') {
			$sql = " SELECT  DISTINCT  P.*, PPIC.resource_id as main_picture_resource_id  from products P
				LEFT OUTER JOIN product_packages PP ON (PP.product_id = P.product_id)
				LEFT OUTER JOIN product_labels PL ON (PL.product_id = P.product_id)
				LEFT OUTER JOIN labels L ON (L.label_id = PL.label_id)
				LEFT OUTER JOIN product_pictures PPIC ON (PPIC.picture_id = P.main_picture)
				LEFT OUTER JOIN suplement_facts SF ON (SF.product_id = P.product_id)
				LEFT OUTER JOIN suplement_fact_items SFI ON (SFI.suplement_fact_id = P.product_id)  WHERE P.status='1' AND  CONCAT_WS(
                ' ',P.sku, P.product_name,P.product_desc,SF.serving_size,SF.other_ingredients, SF.contains,SF.caution,
  L.label_name,SFI.ingredient_text, SFI.ingredient_text_2, SFI.section_name,PP.package_product_no, PP.package_qty, PP.package_medium
  ) LIKE  '%".$kwd."%' AND CONCAT_WS(' ',P.sku, P.product_name,P.product_desc,SF.serving_size,SF.other_ingredients, SF.contains,SF.caution,
  L.label_name,SFI.ingredient_text, SFI.ingredient_text_2, SFI.section_name,PP.package_product_no, PP.package_qty, PP.package_medium
  ) RLIKE '[[:<:]]".$kwd."[[:>:]]'";
				...
				$sql = "SELECT DISTINCT  P.* FROM products P WHERE P.status= '1' ";
			
			
			}
			
			$sql .= " ".$cond." AND P.product_name NOT LIKE 'Extreme Edge%'";
			return  $this->cntNumRows($sql);
					
		}
		
		//function for getting product searched details 
		public function getSearchedProductsDetail($page_cnt,$cond,$kwd) {
		
			if(!empty($kwd)) {	
			$qry = " SELECT  DISTINCT  P.*, PPIC.resource_id as main_picture_resource_id  from products P
				LEFT OUTER JOIN product_packages PP ON (PP.product_id = P.product_id)
				LEFT OUTER JOIN product_labels PL ON (PL.product_id = P.product_id)
				LEFT OUTER JOIN labels L ON (L.label_id = PL.label_id)
				LEFT OUTER JOIN product_pictures PPIC ON (PPIC.picture_id = P.main_picture)
  ...
  L.label_name,SFI.ingredient_text, SFI.ingredient_text_2, SFI.section_name,PP.package_product_no, PP.package_qty, PP.package_medium
				...
				$qry = "SELECT DISTINCT  P.* FROM products P WHERE P.status= '1' ";
